...
const onSubmit = async (data: any) => {
    try {
        ...
        // Anexar documentos
        const documents = data.documents || [];
        console.log("Documentos a serem enviados:",   );
        ...
        const uploadedDocuments = await Promise.all(documents.map(async (doc: any) => {
            if (doc.file) {
                try {
                    ...
                    console.log(`Upload attachmentNumber ${doc.number}`);
                    console.log(`Upload attachmentType ${doc.type}`);
                    console.log(`Upload attachmentUrl ${uploadedFile.url}`);
                    console.log(`Upload attachmentUrl ${uploadedFile.id}`);
                    
                    return {
                      
                        [`attachmentNumber${doc.type}`]: doc.number,
                        
                        [`attachmentType${doc.type}`]: doc.type,
                        
                        [`attachmentUrl${doc.type}`]: uploadedFile.url, 
                        
                        [`attachmentId${doc.type}`]: uploadedFile.id
                    };
                } catch (error) {
                    console.error(`Erro ao fazer upload do documento ${doc.number}:`, error);
                    open({
        ...
        // Filtra documentos que falharam no upload e remove propriedades desnecessárias
        /*data.documents = uploadedDocuments
            ...
            .map(doc => ({
                ...
                [`attachmentUrl${doc.type}`]: doc.url, 
                
                [`attachmentId${doc.type}`]: doc.id
            }));*/
        data.documents = uploadedDocuments
            .filter(doc => doc !== null)
            .map(doc => ({
